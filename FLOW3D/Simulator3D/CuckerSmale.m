...
global N T t x y z p q r D v0 wpc Others


for i=1:N
    Aij(i,i)=1;
    for j=i+1:N
        Aij(i,j)=1/(1+(x(t,i)-x(t,j))^2+(y(t,i)-y(t,j))^2+(z(t,i)-z(t,j))^2)^(b);
        Aij(j,i)=Aij(i,j);
    end
end

u=[p(t,:)-p(t,n);q(t,:)-q(t,n);r(t,:)-r(t,n)]*Aij(:,n)+[p(t,n);q(t,n);r(t,n)];
u=VecNorm(u);
